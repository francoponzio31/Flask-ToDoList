...
from .db_connection import validate_existing_user, validate_user_password
...
class LogInForm(FlaskForm):
    ...
    def validate_username(self, username):
        if not validate_existing_user(username.data):
            raise ValidationError("No existe ningún usuario registrado con ese nombre.")

    def validate_password(self, password):
        if validate_existing_user(self.username.data) and not validate_user_password(self.username.data, password.data):
...
class SignUpForm(FlaskForm):
    ...
    def validate_username(self, username):
        if validate_existing_user(username.data):
